...
namespace Ginger.Models.Formats.ChatLogs
{
	public class TextGenWebUIChat
	{
		...
		public static TextGenWebUIChat FromChat(ChatHistory chatHistory)
		{
			...
			{
				var greeting = chatHistory.messages[0].text;
				lsInternal.Add(new string[2] {"<|BEGIN-VISIBLE-CHAT|>", greeting });
				lsVisible.Add(new string[2] { "", greeting });
				iMsg = 1;
			}
		...
		public static TextGenWebUIChat FromJson(string json)
		{
			try
			{
				...
				if (jObject["internal"] != null && jObject["visible"] != null)
				{
		...
		public static bool Validate(string jsonData)
		{
			try
			{
				...
				return jObject["internal"] != null && jObject["visible"] != null;
			}
			catch
			{
				return false;
