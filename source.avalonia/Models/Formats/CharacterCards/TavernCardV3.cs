...
#pragma warning restore 0414 // Remove unread private members
		...
		public static TavernCardV3 FromOutput(Generator.Output output)
		{
			...
			if (Current.Card.sources != null && Current.Card.sources.Count > 0)
			{
				...
				for (int i = 0; i < lines.Length; ++i)
				{
					...
					if (line.Length > 0 && line[0] == '#')
					{
						...
						if (pos_colon != -1)
						{
							...
							// Change current locale (simplified - just accept the code)
		...
		public static TavernCardV3 FromJson(string json, out int errors)
		{
			...
			try
			{
				...
				// Simplified validation - just check spec field
				var specValue = jObject["spec"]?.ToString();
				if (specValue == "chara_card_v3")
				{
