...
/// <summary>
/// Parser for plain text chat format.
/// </summary>
public class TextFileChat
{
    ...
    public static TextFileChat? FromChat(ChatInstance chatInstance, string[] names)
    {
        ...
        if (names.Length > 2 && !chatInstance.history.isEmpty) // Party
        {
    ...
    public ChatHistory? ToChat()
    {
        ...
        int idxUser = speakers.IndexOfAny(new[] { "User", "Me", "You" });
        if (idxUser != -1)
        {
        ...
        {
            if (!indexById.TryGetValue(message.name ?? "", out int speakerIdx))
                return null; // Error
