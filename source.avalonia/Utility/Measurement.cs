...
namespace Ginger
{
	public static class Measurement
	{
		...
		public static bool Parse(string valueString, NumberParameter.Mode mode, out decimal magnitude, out string unit, out UnitSystem unitSystem)
		{
			magnitude = 0.0m;
			...
			// Normalize units
			if (mode == NumberParameter.Mode.Length || mode == NumberParameter.Mode.Unknown)
			{
				switch (firstUnit)
				{
				case "m":
				...
				case "yards":
					...
					break;
				default:
					if (mode == NumberParameter.Mode.Length)
					{
						firstUnit = "cm";
			...
			else if (mode == NumberParameter.Mode.Weight || mode == NumberParameter.Mode.Unknown)
			{
				switch (firstUnit)
				{
				case "kg":
				...
				case "pounds":
					...
					break;
				default:
					if (mode == NumberParameter.Mode.Weight)
					{
						firstUnit = "kg";
						unitSystem = UnitSystem.Default;
					}
			...
			else if (mode == NumberParameter.Mode.Volume || mode == NumberParameter.Mode.Unknown)
			{
				switch (firstUnit)
				{
				case "ml":
				...
				case "gallons":
					...
					break;
				default:
					if (mode == NumberParameter.Mode.Volume)
					{
						firstUnit = "ml";
						unitSystem = UnitSystem.Default;
					}
		...
		public static string ConvertToImperial(string value)
		{
			...
			if (Parse(value, NumberParameter.Mode.Unknown, out magnitude, out unit, out unitSystem))
			{
				if (unitSystem == UnitSystem.Imperial)
					return value;
