	...
	public class GingerChatV1
	{
		...
		public static GingerChatV1 FromBackup(BackupData.Chat backup)
		{
			...
			if (backup.history?.messages != null)
			{
				...
				{
					if (message.swipes != null && message.swipes.Length > 1)
					{
						lsMessages.Add(new Message() {
							speakerId = message.speaker < speakers.Count ? speakers[message.speaker].id : "0",
							text = message.text,
			...
			var chat = new GingerChatV1() {
				title = backup.name,
				speakers = speakers,
				createdAt = DateTimeExtensions.ToUnixTimeMilliseconds(backup.creationDate),
		...
		{
			if (speakers == null || speakers.Count == 0 || text == null)
				return;
		...
		public static GingerChatV1 FromJson(string json)
		{
			try
			{
				...
				if (jObject["speakers"] != null && jObject["messages"] != null)
				{
		...
		public static bool Validate(string jsonData)
		{
			try
			{
				...
				return jObject["speakers"] != null && jObject["messages"] != null;
			}
			catch
			{
				return false;
