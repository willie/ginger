	...
	public class BackyardChatBackupV1
	{
			...
			for (; iMsg < chat.history.messages.Length; ++iMsg)
			{
				...
				if (message.speaker == 0) // Is user
				{
					...
					if (iMsg > 0 && chat.history.messages[iMsg - 1].speaker == 0)
					{
						lsEntries.Add(new ChatItem() {
							input = lastMessage,
							output = "",
							timestamp = DateTimeExtensions.ToUnixTimeMilliseconds(chat.history.messages[iMsg - 1].creationDate),
						});
					}
				...
				else // Is character
				{
					lsEntries.Add(new ChatItem() {
						input = lastMessage ?? "",
						output = message.text ?? "",
						timestamp = DateTimeExtensions.ToUnixTimeMilliseconds(message.creationDate),
					});
					lastMessage = null;
				}
			}

			backup.name = chat.name ?? "Chat";
			backup.createdAt = DateTimeExtensions.ToUnixTimeMilliseconds(chat.creationDate);
			backup.updatedAt = DateTimeExtensions.ToUnixTimeMilliseconds(chat.updateDate);
		...
		public static BackyardChatBackupV1 FromJson(string json)
		{
			try
			{
				...
				if (jObject["version"] != null && jObject["chat"] != null)
				{
		...
		public static bool Validate(string jsonData)
		{
			try
			{
				...
				return jObject["version"] != null && jObject["chat"] != null;
			}
			catch
			{
				return false;
